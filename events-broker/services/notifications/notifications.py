...
def log_confirmation(user_update: dict[str, Any]) -> None:
    ...
    print(
        f"[notificaciones] received {ROUTING_KEY_USER_UPDATED}: {json.dumps(user_update, ensure_ascii=False)}"
    )
    print(
...
def run_notifications_service(mode: str = "run") -> None:
    ...
    processed = consume_once(
        QUEUE_NOTIFICATIONS, ROUTING_KEY_USER_UPDATED, log_confirmation
    )
    if not processed:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="NovaLink notification service example"
    )
    parser.add_argument(
        "action",
        nargs="?",
