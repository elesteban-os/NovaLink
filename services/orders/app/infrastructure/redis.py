"""Redis helper for the Orders microservice.
...
def is_event_processed(event_id: str, service_name: str = "orders") -> bool:
    client = get_redis_client()
    return client.exists(_processed_key(service_name, event_id)) == 1


def acquire_processing_lock(event_id: str, ttl: int = 30, service_name: str = "orders") -> bool:
    client = get_redis_client()
    return client.set(_processing_key(service_name, event_id), "1", nx=True, ex=ttl) is True


def mark_event_processed(event_id: str, expire_seconds: int = 86400, service_name: str = "orders") -> None:
    client = get_redis_client()
