...
def test_login_endpoint_with_valid_credentials(client, db_session, test_user):
    ...
    log_info("[TEST] Ejecutando: test_login_endpoint_with_valid_credentials -> POST /auth/login")
    ...
    log_assert_equal(200, response.status_code, "status_code for POST /auth/login with valid credentials")
    data = response.json()
...
def test_login_endpoint_with_invalid_password(client, db_session, test_user):
    ...
    log_assert_equal(401, response.status_code, "status_code for invalid password is 401")
    data = response.json()
    log_assert_equal("Incorrect email or password", data["detail"], "detail message for invalid password")
...
def test_login_endpoint_with_nonexistent_user(client):
    ...
    log_assert_equal(401, response.status_code, "status_code for non-existent user is 401")
    data = response.json()
    log_assert_equal("Incorrect email or password", data["detail"], "detail message for non-existent user")
...
def test_login_endpoint_with_missing_password(client):
    ...
    log_assert_equal(422, response.status_code, "status_code for missing password is 422")
...
def test_login_endpoint_with_invalid_email_format(client):
    ...
    log_assert_equal(422, response.status_code, "status_code for invalid email format is 422")
