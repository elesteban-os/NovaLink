...
def test_create_user_service(db_session):
    ...
    log_info(
        "[TEST] Ejecutando: test_create_user_service -> crear usuario a través del servicio"
    )
...
def test_get_user_service_not_found_raises_error(db_session):
    ...
    log_assert_equal(
        "User not found", str(exc_info.value), "error message for not found"
    )
...
def test_remove_nonexistent_skill_from_service_raises_error(db_session):
    ...
    log_info(
        "[TEST] Ejecutando: test_remove_nonexistent_skill_from_service_raises_error"
    )
    ...
    log_assert_equal(
        "Skill not found", str(exc_info.value), "error message for not found"
    )
