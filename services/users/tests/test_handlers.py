...
def test_list_users_endpoint_with_pagination(client, db_session):
    ...
    log_assert_equal(200, response.status_code, "status_code for GET /users with pagination")
    data = response.json()
    log_assert_equal(True, isinstance(data, list), "response is a list")
...
def test_get_nonexistent_user_by_id(client):
    ...
    log_assert_equal(404, response.status_code, "status_code for non-existent user is 404")
...
def test_add_user_skill_endpoint(client, db_session):
    ...
    log_info("[TEST] Ejecutando: test_add_user_skill_endpoint -> POST /users/{id}/skills")
    ...
    log_assert_equal(201, response.status_code, "status_code for POST /users/{id}/skills")
    data = response.json()
...
def test_get_user_skills_endpoint(client, db_session):
    ...
    log_info("[TEST] Ejecutando: test_get_user_skills_endpoint -> GET /users/{id}/skills")
    ...
    for skill in ["Python", "FastAPI", "React"]:
        client.post(
            f"/users/{user_id}/skills",
            json={"skill_name": skill, "points": 5}
        )
    
    # Get skills
    response = client.get(f"/users/{user_id}/skills")
    
    log_assert_equal(200, response.status_code, "status_code for GET /users/{id}/skills")
    data = response.json()
...
def test_remove_user_skill_endpoint(client, db_session):
    ...
    log_info("[TEST] Ejecutando: test_remove_user_skill_endpoint -> DELETE /users/{id}/skills/{name}")
    ...
    client.post(
        f"/users/{user_id}/skills",
        json={"skill_name": "Kubernetes", "points": 8}
    )
    
    # Remove the skill
    response = client.delete(f"/users/{user_id}/skills/Kubernetes")
    
    log_assert_equal(204, response.status_code, "status_code for DELETE /users/{id}/skills/{name}")
...
def test_remove_nonexistent_skill(client, db_session):
    ...
    log_assert_equal(404, response.status_code, "status_code for non-existent skill is 404")
