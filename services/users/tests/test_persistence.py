...
def test_create_user_persistence(db_session):
    ...
    log_info(
        "[TEST] Ejecutando: test_create_user_persistence -> crear usuario en base de datos"
    )
...
def test_get_user_by_id(db_session):
    ...
    log_assert_equal(
        created.email, retrieved.email, "retrieved.email matches created.email"
    )
...
def test_get_user_by_email_nonexistent(db_session):
    ...
    log_assert_equal(
        None, result, "get_user_by_email returns None for non-existent email"
    )
...
def test_update_user_password(db_session):
    ...
    log_assert_equal(
        True, updated.hashed_password != old_hash, "hashed_password changed"
    )
...
def test_remove_nonexistent_skill(db_session):
    ...
    log_assert_equal(
        False, result, "remove_user_skill returns False for non-existent skill"
    )
